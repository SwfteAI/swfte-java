...
public class Conversations {
    ...
    /**
     * Get conversation context optimized for LLM continuation.
     * Returns the most recent messages that fit within token limit.
     *
     * @param conversationId the conversation ID
     * @return list of context messages
     */
    @SuppressWarnings("unchecked")
    public List<ConversationMessage> getContext(String conversationId) {
        return getContext(conversationId, 4096, true);
    }

    /**
     * Get conversation context with options.
     *
     * @param conversationId the conversation ID
     * @param maxTokens maximum tokens to include
     * @param includeSystem whether to include system prompt
     * @return list of context messages
     */
    @SuppressWarnings("unchecked")
    public List<ConversationMessage> getContext(String conversationId, int maxTokens, boolean includeSystem) {
        String url = getBaseUrl() + "/" + conversationId + "/context"
            + "?maxTokens=" + maxTokens
            + "&includeSystem=" + includeSystem;

        Map<String, Object> response = httpClient.getWithCustomBase(url, Map.class);
        return (List<ConversationMessage>) response.get("messages");
    }

    /**
     * Fork a conversation (create a copy from a specific point).
     *
     * @param conversationId the conversation ID
     * @param fromMessageId message ID to fork from (null for all messages)
     * @param newTitle title for the forked conversation
     * @return the forked conversation
     */
    public Conversation fork(String conversationId, String fromMessageId, String newTitle) {
        Map<String, Object> payload = new HashMap<>();
        if (fromMessageId != null) {
            payload.put("fromMessageId", fromMessageId);
        }
        if (newTitle != null) {
        ...
        return httpClient.postWithCustomBase(
            getBaseUrl() + "/" + conversationId + "/fork",
            payload,
            Conversation.class
        );
    }
     ...
     */
    public String export(String conversationId, String format) {
        return httpClient.getWithCustomBase(
            getBaseUrl() + "/" + conversationId + "/export?format=" + format,
            String.class
        );
    }
     ...
     *
     * @param conversationId the conversation ID
     * @return summary as a map containing title and summary fields
     */
    @SuppressWarnings("unchecked")
    public Map<String, String> summarize(String conversationId) {
        return httpClient.postWithCustomBase(
            getBaseUrl() + "/" + conversationId + "/summarize",
            new HashMap<>(),
            Map.class
        );
    }
